...
def prune_model_backward(model, layer):
    ...
    Args:
        ...
        layer: The last layer to keep (0-indexed).
    """
    # Access the encoder layers
    encoder_layers = model.wav2vec2.encoder.layers
    # Prune layers after the specified layer
    pruned_layers = torch.nn.ModuleList(encoder_layers[:layer+1])
...
def main(data_dir, mapping_json, model_dir, output_dir):
    ...
    # Load label2id and vice versa dictionaries
    ...
    # Prune the model to keep only the first 4 layers
    model = prune_model_backward(model, layer=6)
...
if __name__ == "__main__":
    import sys
    if "ipykernel_launcher" in sys.argv[0]:  # Running in Jupyter or similar
        # Default arguments for testing in Jupyter
        data_dir = "processed_data"
        mapping_json = "processed_data/label_mappings.json"
        model_name = "20241119_141957/model"
        output_dir = "./runs"
    else:
    ...
    main(data_dir, mapping_json, model_name, output_dir)
